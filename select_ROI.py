# 원본 사진에서 마우스로 ROI 를 선택하여 좌표를 csv 로 저장하는 소스코드
'''
from select_ROI import ROI2csv

# 1. roi csv 를 저장할 디렉토리 이름 입력하기
ROI2csv = ROI2csv('data/ElectricityMeter_ROI_screen')
# 2. 원본 이미지 디렉토리, 읽어올 파일 인덱스 입력
# 3. 적절한 roi 를 드래그 한 다음 esc 를 눌러 다음 파일 작업
ROI2csv.roi2csv('./data/ElectricityMeter', 0, 20)
...
# 이미지에서 마우스로 ROI 를 추출하고 esc 키를 누르면 좌표가 csv 파일로 저장되는 클래스
class ROI2csv:
